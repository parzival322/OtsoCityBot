...
async def process_career(message: Message, state: FSMContext, bot: Bot):
    if message.text == 'Назад в меню':
        ...
        try:
            bot.send_message(chat_id=1404935980, message=admin_text)
            await message.answer("Спасибо за заявку! В ближайшее время вам ответят в лс! ❤", reply_markup=kb.return_to_menu)
        except Exception as e:
            bot.send_message(chat_id=1404935980, message=f"Отпиши Феде, произошла какая-то ошибка при ЗАПОЛНЕНИИ АНКЕТЫ у игрока @{message.from_user.username} (ID: {message.from_user.id})")
